...
const ClaimReconciliation = () => {
  const [selectedTransaction, setSelectedTransaction] = useState<ClaimTransaction | null>(null);
  const [matchDrawerOpen, setMatchDrawerOpen] = useState(false);
  const [selectedPayor, setSelectedPayor] = useState<string>("All Payors");
  const [selectedType, setSelectedType] = useState<PaymentType | "All Types">("All Types");
  const [selectedStatus, setSelectedStatus] = useState<MatchStatus | "All Status">("All Status");

  const stats = getClaimStats();

  const uniquePayors = ["All Payors", ...new Set(claimTransactions.map((t) => t.payor))];
  const typeOptions: (PaymentType | "All Types")[] = ["All Types", "ACH", "CHECK", "WIRE"];
  const statusOptions: (MatchStatus | "All Status")[] = ["All Status", "Matched", "Needs Match"];
  ...
  const filteredTransactions = claimTransactions.filter((t) => {
    const payorMatch = selectedPayor === "All Payors" || t.payor === selectedPayor;
    const typeMatch = selectedType === "All Types" || t.type === selectedType;
    const statusMatch = selectedStatus === "All Status" || t.matchStatus === selectedStatus;
    ...
    <DashboardLayout>
      ...
      >
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-foreground">Claim Reconciliation</h1>
      ...
      >
        ...
        <div className="bg-card rounded-xl border border-border shadow-soft p-5">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {stats.matchPercentage}% of transactions matched · {stats.matchedCount} matched
          </p>
        </div>
      </motion.div>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start" className="bg-background">
            {uniquePayors.map((payor) => (
              <DropdownMenuItem key={payor} onClick={() => setSelectedPayor(payor)}>
                {payor}
              </DropdownMenuItem>
            ))}
          </DropdownMenuContent>
      ...
      >
        ...
        <div className="grid grid-cols-[80px_180px_1fr_80px_120px_120px] gap-4 px-6 py-4 border-b border-border">
          ...
          <div className="text-xs font-normal text-muted-foreground text-right">Amount</div>
          <div className="text-xs font-normal text-muted-foreground">Match Status</div>
        </div>
        ...
        <div className="divide-y divide-border">
          {paginatedItems.map((transaction, index) => (
            ...
            >
              ...
              <div className="text-sm font-medium text-foreground">{transaction.payor}</div>
              <div className="text-sm text-muted-foreground">{transaction.description}</div>
              <div className="text-xs text-muted-foreground uppercase">{transaction.type}</div>
              ...
              <div className="flex items-center gap-1.5">
                {transaction.matchStatus === "Matched" ? (
                  ...
                  <span className="text-sm text-category-coral font-medium">Needs Match</span>
                )}
              </div>
            </motion.div>
          ))}
...
export default ClaimReconciliation;
