...
import { Search, DollarSign, FileText, ClipboardList, Settings, ChevronDown } from "lucide-react";
import { DashboardLayout } from "@/components/layout/DashboardLayout";
import { Input } from "@/components/ui/input";
import { MailDetail } from "@/components/mailbox/MailDetail";
import { mailItems, MailItem, MailType, MailStatus } from "@/data/mailbox";
...
const typeOptions: (MailType | "All Types")[] = ["All Types", "Check", "EOB", "Medical Records", "Refund", "Other"];
const statusOptions: (MailStatus | "All Status")[] = ["All Status", "PENDING", "PROCESSED"];

const DigitalMailbox = () => {
  const [selectedMail, setSelectedMail] = useState<MailItem | null>(null);
  const [detailOpen, setDetailOpen] = useState(false);
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedType, setSelectedType] = useState<MailType | "All Types">("All Types");
  const [selectedStatus, setSelectedStatus] = useState<MailStatus | "All Status">("All Status");
  ...
  const filteredMail = mailItems.filter((mail) => {
    ...
    const typeMatch = selectedType === "All Types" || mail.type === selectedType;
    const statusMatch = selectedStatus === "All Status" || mail.status === selectedStatus;
  ...
  return (
    <DashboardLayout>
      ...
      <motion.div
        ...
        className="mb-6"
      >
        <h1 className="text-2xl font-semibold text-foreground">Digital Mailbox</h1>
      ...
      >
        ...
        <div className="grid grid-cols-[200px_200px_1fr_140px] gap-4 px-6 py-4 border-b border-border">
          <div className="text-xs font-normal text-muted-foreground">Received</div>
          <div className="text-xs font-normal text-muted-foreground">Type</div>
          <div className="text-xs font-normal text-muted-foreground">Sent By</div>
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-medium text-foreground">{mail.type}</span>
                </div>
                <div className="text-sm font-medium text-foreground">{mail.sentBy}</div>
                <div
                  className={cn(
