...
const InsurancePaymentsPage = () => {
  const [selectedPayment, setSelectedPayment] = useState<InsurancePayment | null>(null);
  ...
  const uniquePayors = ["All Payors", ...new Set(insurancePayments.map((p) => p.payor))];
  ...
  const filteredPayments = insurancePayments.filter((p) => {
    ...
    // Filter by payor
    const payorMatch = selectedPayor === "All Payors" || p.payor === selectedPayor;
    ...
    <DashboardLayout>
      ...
      >
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-foreground">Insurance Payments</h1>
      ...
      >
        ...
        <div className="grid grid-cols-[48px_140px_1fr_140px_140px_1fr_140px] gap-4 px-6 py-4 border-b border-border">
          ...
          <div className="text-xs font-normal text-muted-foreground">Settlement Date</div>
          <div className="text-xs font-normal text-muted-foreground">Payor</div>
          <div className="text-xs font-normal text-muted-foreground">Payor ID</div>
          <div className="text-xs font-normal text-muted-foreground">Payment Method</div>
          <div className="text-xs font-normal text-muted-foreground">Payment Reference</div>
            ...
            >
              ...
              >
                <Checkbox
                  ...
                  onCheckedChange={(checked) => handleCheckboxChange(payment.id, checked as boolean)}
                />
              </div>
              <div className="text-sm text-muted-foreground">{payment.settlementDate}</div>
              <div className="text-sm font-medium text-foreground">{payment.payor}</div>
              <div className="text-sm text-muted-foreground">{payment.payorId}</div>
              <div className="text-sm text-muted-foreground">{payment.paymentMethod}</div>
              <div className="text-sm text-muted-foreground">
                <span className="text-muted-foreground">{payment.referenceType}</span>
                <br />
                <span className="text-foreground">{payment.paymentReference}</span>
              </div>
